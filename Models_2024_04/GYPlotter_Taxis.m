...
save_path = 'Traj_Color_f1/'; % Ruta corregida
...
for f_tax = f_tax_values
    for H = H_values
        ...
        for k = 1:length(dr_folders)
            ...
            x_org = -100 / sqrt(2);
            % scatter(x_org, x_org, 60, 'filled', 'k'); % Punto organizador en negro

            for j = 1:100
                final_position = data{j}(end, :); % Última posición de cada archivo
                scatter(final_position.x_array, final_position.y_array, 20, [0 0.4470 0.7410],'filled');
            end
            ...
            filename = fullfile(save_path, ['GY_Alpha_0__f_', f_tax_str, '_Dr_', Dr_str, '_H_', H_str, '.png']);

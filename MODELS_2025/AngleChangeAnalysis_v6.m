...
for Hval = 1:length(H_values)
    ...
    for Drval = 1:length(Dr_values)  
        ...
        % figure;
        % edges = [0:pi/50:pi];
        % histogram(deg2rad(file_angle_full),edges)
        % axis ([0 pi 0 5*10^4])
        % axis on;
        % box on;
        % set(gca,'LineWidth',3)
        ...
        % % Saving
        % filename = ['../Model1_H_cnst/FIGURES_082024/' ...
        %     'HistogramAngle/Alpha_0_Dr_', Dr_str, '__H_',H_str,'__Histogramnglesize.png'];
        % exportgraphics(gcf,filename,'Resolution',300);
        % close(gcf);
        ...
        figure;
        plot(time_vector(3:998),(mean(file_angle_full(3:998,:),2)),'LineWidth', 2);
        hold off;
        axis ([0 1000 0 pi])
        ...
        % Saving
        filename = ['../Model1_H_cnst/FIGURES_082024/' ...
            'MeanEVOAngle/Alpha_0_Dr_', Dr_str, '__H_',H_str,'__TimeEVOMEANAnglesize.png'];
        exportgraphics(gcf,filename,'Resolution',300);
        close(gcf);
    end
end

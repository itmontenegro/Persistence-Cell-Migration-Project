...
mean_noise_norm_matrix = zeros(length(Dr_values),200);

mean_noise_persfac_quotient_matrix = zeros(length(Dr_values),200);
    ...
    for Drval = 1:length(Dr_values)  
        ...
        for sim = 1:200
            ...
            filename = ['Model1_H_cnst/FIGURES_082024/' ...
                'Scatter_PF_IC/Alpha_0_25_Dr_', Dr_str, '__H_',H_str,'__PF_IC.png'];            
            % close(gcf);
            hold on;
        end
        hold off;
...
%% STEPSIZE VS PERSFAC
noise_norm_matrix = sqrt(noise_matrix_x.^2 + noise_matrix_y.^2);
...
for Hval = 1:length(H_values)
    ...
    for Drval = 1:length(Dr_values)
        ...
        Dr_value = Dr_values(Drval)
        Dr_str = strrep(num2str(Dr_value), '.', '_');
        step = mean_stepsize_matrix(Drval,:);
        persfac = mean_persfac_matrix(Drval,:);
        noise_norm_row = noise_norm_matrix(Drval,:);

        noise_persfac_quotient = persfac ./ noise_norm_row

        scatter(step, noise_persfac_quotient, 'filled')
        ...
        axis([0 max(mean_stepsize_matrix, [], 'all') 0 max(noise_norm_matrix, [], 'all')])
        axis on;
        box on;
        set(gca,'LineWidth',3)
        %set(gca, 'XTickLabel', []);
        ...
        % Saving
        filename = ['Model1_H_cnst/FIGURES_082024/' ...
            'Scatter_PF_IC/Alpha_0_25_H_', H_str, '__PF_IC.png'];            
        % close(gcf);
        hold on;
        exportgraphics(gcf,filename,'Resolution',300);
    end
...
for Hval = 1:length(H_values)
    ...
    for Drval = 1:length(Dr_values)
        ...
        scatter(angle, noise_persfac_quotient, 'filled')

        % title(['\alpha = 0 and H = ', num2str(H_value)])
        % xlabel('Stepsize')
        % ylabel('Relative Angle')
        axis([0 max(mean_angle_matrix, [], 'all') 0 1])
        axis on;
        box on;
        set(gca,'LineWidth',3)
        set(gca, 'XTickLabel', []);
...
% 
% 
%% Initial noise vs initial angle

...
for h in range(len(H_values)):
    for dr in range(len(Dr_values)):
        for f in range(len(Ftax_values)):
            ...
            f_tax_str = str.replace(str(ftax), '.', '_')
            Alpha_str = str.replace(str(alpha), '.', '_')

            # Carpeta de cada combinación de H1 y H2
            folder_path = f'{base_path}/Alpha_{Alpha_str}_f_tax_{f_tax_str}/H_{H_str}/Dr_{Dr_str}/'

            batch = np.load(f'{folder_path}Batch_Dr_{Dr_str}_H_{H_str}_Alpha_{Alpha_str}.npz')
            
            # Revisar que hayan suficientes trayectorias para graficar
            if batch['x_array'].shape[0] < n_traj:
                print(f"Warning: Not enough trajectories in batch for H={H}, Dr={Dr}. Found {batch['x_array'].shape[0]}, expected {n_traj}. Skipping.")
                continue
